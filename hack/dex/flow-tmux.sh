...
if [ "$skip_research" = "false" ]; then
  # research
  tmux rename-window "AI-${number}-research"
...
# linear
tmux rename-window "AI-${number}-linear"
claude "/linear review all the files in thoughts/ (research, plans) related to AI-${number} and attach to the ticket links, then put the ticket in 'spec in review' state"
sleep 5
