...
function extractIssueId(branchName: string): string | null {
  // Match patterns like AI-123, eng-123, etc.
...
async function getIssueIdInteractively(
  ...
  const { issueId } = await inquirer.prompt({
    type: "input",
    name: "issueId",
    message: "Enter Linear issue ID (e.g. AI-123):",
    default: defaultId,
    validate: (input) => {
      return /^[A-Za-z]+-\d+$/i.test(input)
        ? true
        : "Please enter a valid issue ID (e.g. AI-123)";
    },
  });
  ...
  try {
    ...
    if (comments.nodes.length > 0) {
      ...
      for (const comment of reversedComments) {
        ...
        console.log(
          chalk.dim(
            `[${dateStr} ${timeStr}] ${commentUser?.name || "Unknown"}:`,
          ),
        );
...
async function updateStatus(
  ...
  try {
    ...
    if (!issueId || !/^[A-Za-z]+-\d+$/i.test(issueId)) {
      console.error(
        chalk.red("Error: Invalid issue ID format. Expected format: AI-123"),
      );
      process.exit(1);
    }
    ...
    if (result.success) {
      console.log(
        chalk.green(
          `✓ Updated ${normalizedId} status to "${targetState.name}"`,
        ),
      );
    } else {
...
async function fetchImages(issueId: string): Promise<void> {
  try {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      // Generate filename: AI-1234-01.png format
